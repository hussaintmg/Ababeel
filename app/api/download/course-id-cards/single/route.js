// app/api/download/course-id-cards/single/route.js
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ user: null, loggedIn: false, error: "Invalid token" });
    }

    const user = await User.findById(decoded.id).select("-password");
    if (!user) {
      return NextResponse.json({ user: null, loggedIn: false, error: "User not found" });
    }
    ...
    if (!template || !template.designData?.pages?.length) {
      return NextResponse.json({ success: false, error: "No active Course ID Card template found" }, { status: 404 });
    }
    ...
    const dataMap = {
      ...
      CourseReference: courseReference.toObject(),
    ...
    const title = `${candidate.firstName} ${candidate.lastName} - ${courseReference.courseName} Id Card`;
    ...
    return new NextResponse(pdfBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="CourseIdCard-${candidate.traineeId}.pdf"`,
