...
function withCustomNav(settings, extra, hidden = []) {
  ...
  // A dropdown's children are links to hidden pages just as much as a
  // top-level link is, so they are filtered too. A parent left with no children
  // and no url of its own has nothing to open, so it goes as well.
  const prune = (link) => {
    if (!Array.isArray(link.dropdown) || !link.dropdown.length) return link;
    const dropdown = link.dropdown.filter((child) => !isHidden(child.url));
    if (dropdown.length === link.dropdown.length) return link;
    return { ...link, dropdown };
  };

  const visible = merged
    .filter((l) => !isHidden(l.url))
    .map(prune)
    .filter((l) => l.url || l.dropdown?.length);

  const unchanged =
    !extra?.length &&
    visible.length === base.length &&
    visible.every((l, i) => l === base[i]);
