...
/** A published course by id or slug — used by registration and public pages. */
export async function getPublicCourseById(id) {
  if (!id) return null;
  const str = String(id).trim();
  if (!mongoose.Types.ObjectId.isValid(str)) {
    return getPublicCourseBySlug(str);
  }
  try {
    await connectDB();
    const course = await Course.findOne({ _id: str, ...PUBLISHED })
    ...
    return course ? plain(course) : (await getPublicCourseBySlug(str));
  } catch (error) {
...
export async function listTestimonials(limit = 20) {
  try {
    await connectDB();
    const rows = await Testimonial.find(PUBLISHED)
      .select("name profileImage reviewText rating company position reviewDate sourceLogo sourceName verifiedLabel sourceUrl featured displayOrder")
      .sort({ featured: -1, displayOrder: 1, createdAt: -1 })
      .limit(Number(limit) || 20)
      .lean();
    return plain(rows);
  } catch (error) {
    console.error("listTestimonials failed:", error?.message);
    return [];
  }
}
...
export async function listTeamMembers(limit = 50) {
  try {
    await connectDB();
    const rows = await TeamMember.find(PUBLISHED)
      .select("name slug position profileImage bio qualifications certifications experience email socialLinks leadership status displayOrder")
      .sort({ leadership: -1, displayOrder: 1, name: 1 })
...
export async function listConsultants(limit = 50) {
  try {
    await connectDB();
    const rows = await Consultant.find(PUBLISHED)
      .select("name slug position profileImage gallery bio qualifications certifications experience expertise socialLinks layout showCarousel textAlign animation featured status displayOrder")
      .sort({ featured: -1, displayOrder: 1, name: 1 })
      .limit(Number(limit) || 50)
      .lean();
    return plain(rows);
  } catch (error) {
    console.error("listConsultants failed:", error?.message);
    return [];
  }
}
...
export async function getConsultantBySlug(slug) {
  ...
  try {
    await connectDB();
    const row = await Consultant.findOne({
...
export async function listPublicResources({
  ...
  try {
    ...
    if (search && search.trim()) {
      ...
      query.$or = [{ title: rx }, { shortDescription: rx }, { content: rx }];
    }
    ...
    const [total, rows] = await Promise.all([
      ...
      Resource.find(query)
        .select("title slug shortDescription content featuredImage type file fileLabel externalUrl publishedDate featured status displayOrder")
        .sort({ featured: -1, publishedDate: -1, displayOrder: 1, createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .lean(),
    ]);
...
export async function getResourceTypes() {
  try {
    await connectDB();
    const distinctTypes = await Resource.distinct("type", PUBLISHED);
    if (distinctTypes && distinctTypes.length > 0) {
      return distinctTypes;
    }
    return Array.from(RESOURCE_TYPES || []);
  } catch (error) {
...
export async function getRelatedResources(resource, limit = 3) {
  ...
  try {
    ...
    const rows = await Resource.find(query)
      .select("title slug shortDescription featuredImage type file fileLabel externalUrl publishedDate featured")
      .sort({ featured: -1, publishedDate: -1, displayOrder: 1 })
