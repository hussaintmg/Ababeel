...
/** Query fragment: only published & active catalogue entries. */
export const PUBLISHED = { status: { $in: ["published", "active"] } };
...
export function isCoursePublic(course) {
  return !!course && (course.status === "published" || course.status === "active");
}
