...
const PLACEHOLDER_IMAGE = "/ababeel-logo.svg";
...
function sampleScalar(field, key, index) {
  ...
  switch (type) {
    ...
    default: {
      ...
      if (/position|jobtitle|role/.test(name)) return "Senior Safety Specialist";
      if (/company|organisation|sourcename/.test(name)) return "Apex Industrial Solutions";
      if (/verifiedlabel/.test(name)) return "Verified Review";
      if (/currency$/.test(name)) return "GBP";
      if (/symbol/.test(name)) return "£";
      if (/description|message|bio|summary|about|reviewtext|content/.test(name)) return pick(SENTENCES, key, index);
      if (/status/.test(name)) return "published";
