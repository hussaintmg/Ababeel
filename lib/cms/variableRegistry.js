...
export async function syncVariables({ email = "" } = {}) {
  ...
  // Prune any stored schema variables that no longer exist in the schema registry
  await CmsVariable.deleteMany({
    kind: "schema",
    name: { $nin: Array.from(discoveredNames) },
  });

  // Upsert all discovered variables into the database so MongoDB has a live, complete registry
  if (discovered.length > 0) {
    const ops = discovered.map((v) => ({
      updateOne: {
        filter: { name: v.name, kind: "schema" },
        update: {
          $set: {
            label: v.label,
            type: v.type,
    ...
    await CmsVariable.bulkWrite(ops);
  }
