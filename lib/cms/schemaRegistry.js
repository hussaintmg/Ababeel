...
import { getRelationsForModel } from "@/lib/cms/relations";

// How deep we follow `ref:` relationships. 3 allows
// courseReference.course.level.name without risking cycle blow-ups.
...
export function describeModel(model) {
  ...
  const existingPaths = new Set(fields.map((f) => f.name));
  const relations = getRelationsForModel(name);
  for (const rel of relations) {
    if (!existingPaths.has(rel.alias) && !isBlockedField(name, rel.alias)) {
      const relChildrenList = refChildren(rel.targetModel, rel.alias, `${camelize(name)}.${rel.alias}`, 1, new Set([name]));
      fields.push({
        name: rel.alias,
        path: rel.alias,
        fullPath: `${camelize(name)}.${rel.alias}`,
        type: rel.isArray ? VAR_TYPES.Array : VAR_TYPES.Reference,
        ref: rel.targetModel,
        isArray: Boolean(rel.isArray),
        description: `Reference to ${humanizeModel(rel.targetModel)}`,
        children: relChildrenList,
      });
      existingPaths.add(rel.alias);
    }
  }
...
export function isQueryableField(modelName, path) {
  if (modelName === 'TrainingCourse') return ['status', 'name', 'slug', 'title', 'price', '_id'].includes(path);
