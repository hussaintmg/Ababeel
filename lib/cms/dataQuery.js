 ...
 *
 * Three modes: "list" publishes an array, "single" one document, and "count"
 * the number of matching documents — the last so a page can show a real figure
 * ("{{stats.programs}} training programmes") instead of a number typed by hand
 * that drifts out of date the moment the collection changes.
 *
...
export async function runDataSource(source, ctx = {}) {
  ...
  if (!isAllowedModel(modelName)) {
    const failed = source?.mode === "count" ? 0 : source?.mode === "single" ? null : [];
    return { key, mode: source?.mode || "list", model: modelName, data: failed, total: 0, error: `Model "${modelName}" is not available to the CMS` };
  }

  await connectDB();
  const model = mongoose.models[modelName];
  const mode = source?.mode === "single" || source?.mode === "count" ? source.mode : "list";
  ...
  try {
    if (mode === "count") {
      // The number itself is the value, so `{{key}}` renders it directly.
      const n = await model.countDocuments(filter);
      return { key, mode, model: modelName, data: n, total: n, error: null };
    }
    ...
    const empty = mode === "count" ? 0 : mode === "single" ? null : [];
    return { key, mode, model: modelName, data: empty, total: 0, error: "Query failed" };
  }
}
