...
export async function runDataSources(sources, ctx = {}, opts = {}) {
  ...
  while (pending.size > 0 && maxRounds-- > 0) {
    ...
    const tierResults = await Promise.all(
      runnable.map((s) => (depMap.get(s.key)||[]).some(d=>resolvedMeta[d]?.error) ? Promise.resolve({key:s.key,data:null,total:0,error:"Dependency failed"}) : runDataSource(s, currentCtx, opts))
    );
