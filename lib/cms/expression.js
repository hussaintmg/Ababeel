...
const ROOT_ALIASES = {
  courseref: ["courseRef", "courseReferences", "courses", "courseReference", "course"],
  courserefs: ["courseReferences", "courses", "courseRef", "courseReference"],
  coursereference: ["courseReferences", "courseReference", "courses", "courseRef", "course"],
  coursereferences: ["courses", "courseReferences", "courseReference", "courseRef", "course"],
  courses: ["courses", "courseReferences", "courseReference", "courseRef", "course"],
  course: ["course", "courseReferences", "courses", "courseRef", "courseReference"],
  candidate: ["candidate", "candidates"],
  candidates: ["candidates", "candidate"],
  teammember: ["teamMember", "teamMembers"],
 ...
 * Follows strict lexical scope order:
 * 1. Nearest local loop scope (and loop metadata namespace)
 * 2. Parent loop scopes
 * 3. Page data sources
 * 4. Route context (route.params.* / params.*)
 * 5. Site context (site.*)
 * 6. Global context (now)
 * 7. Legacy root aliases (fallback only)
 */
export function getPath(ctx, path) {
  ...
  // 1. Loop metadata namespace: loop.index, loop.first, loop.last, loop.count, etc.
  if (firstPart === "loop" || firstPart === "$loop") {
    const loopMeta = ctx.loop || ctx._loopMeta || {
      index: ctx.index,
      number: ctx.number,
      first: ctx.isFirst,
      last: ctx.isLast,
    ...
    if (remainingParts.length === 0) return loopMeta;
    let cur = loopMeta;
  ...
  // 2. Lexical scope stack (inner loop -> outer loop)
  if (Array.isArray(ctx.__scopeStack) && ctx.__scopeStack.length > 0) {
    for (let i = ctx.__scopeStack.length - 1; i >= 0; i--) {
      const frame = ctx.__scopeStack[i];
      if (frame && typeof frame.resolve === "function") {
        const res = frame.resolve(firstPart, remainingParts);
        if (res.found) return res.value;
      } else if (frame && frame.alias === firstPart) {
        let cur = frame.data;
        for (const p of remainingParts) {
          if (cur === null || cur === undefined) return undefined;
          cur = cur[p];
  ...
  // 4. Route context convenience: route.params.x -> ctx.params.x or ctx.route.params.x
  if (firstPart === "route") {
    const routeObj = ctx.route || { params: ctx.params || {}, query: ctx.query || {} };
    let rCur = routeObj;
    for (const p of remainingParts) {
      if (rCur === null || rCur === undefined) return undefined;
      rCur = rCur[p];
    }
    if (rCur !== undefined) return rCur;
  }

  // 5. Root Aliases (fallback only if key not found directly and no scope matched)
  const aliasKey = String(firstPart).toLowerCase().replace(/[^a-z0-9]/g, "");
  const candidates = ROOT_ALIASES[aliasKey];
  if (candidates) {
    for (const cand of candidates) {
      if (cand in ctx && ctx[cand] !== undefined) {
        let aCur = ctx[cand];
        for (const p of remainingParts) {
          if (aCur === null || aCur === undefined) return undefined;
          if (Array.isArray(aCur)) {
            if (/^\d+$/.test(p)) { aCur = aCur[Number(p)]; continue; }
            if (p === "length") { aCur = aCur.length; continue; }
            if (aCur.length > 0 && typeof aCur[0] === "object") { aCur = aCur[0][p]; continue; }
            return undefined;
          }
          if (typeof aCur !== "object") return undefined;
          aCur = aCur[p];
        }
        return aCur;
      }
...
export const FUNCTIONS = {
  ...
  prefix: (v, p = "") => (v == null || v === "" ? "" : String(p) + String(v)),
  suffix: (v, s = "") => (v == null || v === "" ? "" : String(v) + String(s)),
  percentage: (v, d = 0) => {
    const n = Number(v);
    if (!Number.isFinite(n)) return "";
    const decimals = Math.max(0, parseInt(d, 10) || 0);
...
/**
 * Format and transform values (currency, uppercase, date, count, etc.)
 */
export function formatTransform(value, format, ...args) {
  if (value === null || value === undefined) return "";
  const fmt = String(format || "").toLowerCase().trim();

  // String transforms
  if (fmt === "uppercase" || fmt === "upper") return String(value).toUpperCase();
  if (fmt === "lowercase" || fmt === "lower") return String(value).toLowerCase();
  if (fmt === "capitalize") {
    const s = String(value);
    return s.charAt(0).toUpperCase() + s.slice(1);
  }
  if (fmt === "truncate") {
    const len = parseInt(args[0], 10) || 50;
    const s = String(value);
  ...
  if (fmt === "decimal") {
    const n = Number(value);
    if (!Number.isFinite(n)) return String(value);
    const digits = parseInt(args[0], 10) || 2;
  ...
  if (fmt === "time") {
    const d = new Date(value);
    return Number.isNaN(d.getTime()) ? String(value) : d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" });
  }
