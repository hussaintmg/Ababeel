...
export function getPath(ctx, path) {
  ...
  for (let idx = 0; idx < parts.length; idx++) {
    ...
    // Check root alias if key not found directly on ctx
    if (cur === ctx && !(part in cur)) {
      const aliasKey = String(part).toLowerCase().replace(/[^a-z0-9]/g, "");
      const candidates = ROOT_ALIASES[aliasKey];
      if (candidates) {
        for (const cand of candidates) {
          if (cand in cur && cur[cand] !== undefined) {
            cur = cur[cand];
            break;
          }
        }
      ...
      if (cur !== ctx) {
        // We matched an alias for this part; continue loop if more parts or finish
    ...
    if (Array.isArray(cur)) {
      ...
      // If part exists directly on the array or array prototype
      if (part in cur) {
        cur = cur[part];
        continue;
      }
      // Access property on the first element if cur[0] is an object (gracefully handles array.prop and array[].prop)
