...
export async function resolveDynamicItem(dynamicRoute, paramValue) {
  ...
  const result=await runDataSource({key:itemKey,model:modelName,operation:'routeParam',paramName:'value',lookupField}, {params:{value:paramValue}}, {allowReserved:true});
  return {key:itemKey,item:result.data,error:result.error};
}
