...
/**
 * Three-state compatibility engine:
 * Recommended: direct type match.
 * Convertible: can be safely converted or transformed (e.g. number -> text, array -> count).
 * Incompatible: cannot be bound, returns educational diagnostic reason.
 */
export function getCompatibility(fieldType, varType) {
  if (!fieldType || !varType) return { state: COMPATIBILITY_STATES.RECOMMENDED };
  const targetRaw = String(fieldType).toLowerCase().trim();
  const target = targetRaw === "string" ? "text" : targetRaw;
  const source = String(varType);
  const isArr = isArrayType(source);

  // Collection/Repeater targets
  if (target === "collection" || target === "array" || target === "loop") {
    if (isArr) return { state: COMPATIBILITY_STATES.RECOMMENDED };
    return {
      state: COMPATIBILITY_STATES.INCOMPATIBLE,
      reason: `A repeat or loop requires an Array or Collection source, but this is a ${source}.`,
    };
  }

  // If source is Array but target is a scalar field
  if (isArr) {
    if (target === "number") {
      return {
        state: COMPATIBILITY_STATES.CONVERTIBLE,
        reason: "Array contains multiple items. Can be used with a Count transform.",
        suggestedTransform: "count",
      };
    }
    if (target === "text" || target === "textarea") {
      return {
        state: COMPATIBILITY_STATES.CONVERTIBLE,
        reason: "Array contains multiple items. Use a Loop, Count transform, or Join transform.",
        suggestedTransform: "count",
      };
    }
    return {
      state: COMPATIBILITY_STATES.INCOMPATIBLE,
      reason: `Cannot bind an Array directly to a ${target} property. Create a Loop to iterate over items.`,
    };
  }

  // Text / Textarea targets
  if (target === "text" || target === "textarea") {
    if (source === VAR_TYPES.String || source === VAR_TYPES.RichText) {
      return { state: COMPATIBILITY_STATES.RECOMMENDED };
    }
    if (source === VAR_TYPES.Number || source === VAR_TYPES.Date || source === VAR_TYPES.DateTime || source === VAR_TYPES.Boolean || source === VAR_TYPES.Email || source === VAR_TYPES.URL) {
      return {
        state: COMPATIBILITY_STATES.CONVERTIBLE,
        reason: `${source} can safely be converted to text or formatted with transforms.`,
      };
    }
    if (source === VAR_TYPES.Reference || source === VAR_TYPES.Object) {
      return {
        state: COMPATIBILITY_STATES.CONVERTIBLE,
        reason: `${source} resolves to the record's name or title, or you can pick a specific child property like .name.`,
      };
    }
    return {
      state: COMPATIBILITY_STATES.INCOMPATIBLE,
      reason: `Cannot directly bind ${source} to a text property.`,
    };
  }

  // Image targets
  if (target === "image") {
    if (source === VAR_TYPES.Image || source === VAR_TYPES.URL) {
      return { state: COMPATIBILITY_STATES.RECOMMENDED };
    }
    if (source === VAR_TYPES.String) {
      return {
        state: COMPATIBILITY_STATES.CONVERTIBLE,
        reason: "String might contain an image URL.",
      };
    }
    return {
      state: COMPATIBILITY_STATES.INCOMPATIBLE,
      reason: `${source} is not an image or image URL.`,
    };
  }

  // Number targets
  if (target === "number") {
    if (source === VAR_TYPES.Number) return { state: COMPATIBILITY_STATES.RECOMMENDED };
    if (source === VAR_TYPES.String) {
      return {
        state: COMPATIBILITY_STATES.CONVERTIBLE,
        reason: "String will be parsed as a number if valid.",
      };
    }
    return {
      state: COMPATIBILITY_STATES.INCOMPATIBLE,
      reason: `${source} cannot be converted to a number.`,
    };
  }

  // Boolean targets
  if (target === "boolean") {
    if (source === VAR_TYPES.Boolean) return { state: COMPATIBILITY_STATES.RECOMMENDED };
    return {
      state: COMPATIBILITY_STATES.CONVERTIBLE,
      reason: "Truthy/falsy evaluation.",
    };
  }

  return isCompatible(fieldType, varType)
    ? { state: COMPATIBILITY_STATES.RECOMMENDED }
    : { state: COMPATIBILITY_STATES.INCOMPATIBLE, reason: `${source} cannot be used for ${fieldType}.` };
}
