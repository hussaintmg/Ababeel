...
export function sourceVariables(sources, tree = [], dynamicRoute = null) {
  ...
  for (const s of Array.isArray(sources) ? sources : []) {
    ...
    const mode = s.operation === "count" ? "count" : ["findOne", "findById", "findBySlug", "routeParam", "currentUser"].includes(s.operation) ? "single" : s?.mode === "single" || s?.mode === "count" ? s.mode : "list";
...
/** Describe a lexical loop using an actual configured collection, never a guessed model. */
export function loopVariables(variables, source, alias = "item") {
  const path = String(source || "").replace(/[{}]/g, "").trim();
  const collection = variables.find(v => v.name === path && (v.kind === "source" || v.kind === "loop"));
  if (!collection || !(collection.isArray || /^Array/.test(collection.type))) return [];
  const itemType = /^Array<(.+)>$/.exec(collection.type)?.[1];
  const prefix = `${path}[]`;
  const out = [{ ...collection, name: alias, type: itemType && itemType !== "Reference" ? itemType : "Object", isArray: false, kind: "loop" }];
  for (const v of variables) {
    if (v.name.startsWith(`${prefix}.`)) {
      const name = alias + v.name.slice(prefix.length);
      out.push({ ...v, name, fullPath: name, kind: "loop" });
    }
  }
  for (const name of ["index", "number", "count", "first", "last", "even", "odd"]) {
    out.push({ name: `loop.${name}`, type: ["index", "number", "count"].includes(name) ? "Number" : "Boolean", kind: "loop", category: "Current loop" });
  }
  return out;
}
