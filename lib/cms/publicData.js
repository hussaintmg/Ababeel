...
export async function resolvePublicBlocks(doc, { request = null, params = {}, user = null } = {}) {
  ...
  if (!resolved) {
    const filled = await injectPublicSectionData(await injectTrainingData(blocks), params);
    return { blocks: filled, dynamic: false, context: null, meta: {} };
  }

  const expanded = expandBlocks(blocks, resolved.context);
  return {
    blocks: await injectPublicSectionData(await injectTrainingData(expanded), params),
    dynamic: true,
