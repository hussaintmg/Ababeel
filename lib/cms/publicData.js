import { flattenBlockTree } from "./blockTree";
/**
...
export function hasTrainingBlocks(doc) {
  const blocks = flattenBlockTree(doc?.blocks);
  return blocks.some((b) => TRAINING_BLOCK_TYPES.includes(b?.type) || publicBlockPage(b?.type));
}

/** True when a page actually binds to data (so it must not be cached shared). */
export function hasDynamicContent(doc) {
  if (!doc) return false;
  if (flattenBlockTree(doc.blocks).some(block => {
    const page = publicBlockPage(block.type);
    return page === 'registration' || page?.endsWith('-detail');
  })) return true;
 ...
 */
export async function resolvePublicBlocks(doc, { request = null, params = {}, user = null, resolved: suppliedResolved, strict = false } = {}) {
  const blocks = Array.isArray(doc?.blocks) ? doc.blocks : [];
  const resolved = suppliedResolved !== undefined ? suppliedResolved : await resolvePublicPageData(doc, { request, params, user });
  ...
  if (!resolved) {
    const filled = await injectPublicSectionData(await injectTrainingData(blocks, {strict}), params);
    return { blocks: filled, dynamic: false, context: null, meta: {} };
  }

  const expanded = expandBlocks(blocks, resolved.context);
  return {
    blocks: await injectPublicSectionData(await injectTrainingData(expanded, {strict}), params),
    dynamic: true,
