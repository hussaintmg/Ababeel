...
export function sdkRuntimeData(block, ctx = {}) {
  ...
  for (const match of code.matchAll(/(?:useCMSData\s*\(\s*|source\s*=\s*|CMSLoop[^>]*source\s*=\s*)['"]([^'"]+)['"]/g)) roots.add(match[1].split(/[.\[]/)[0]);
  for (const path of block.props?._dataDependencies || []) if (typeof path === "string") roots.add(path.split(/[.\[]/)[0]);

  // If the component code mentions any key present in ctx, include it in runtime data
  if (ctx && typeof ctx === "object") {
    for (const key of Object.keys(ctx)) {
      if (!key || key.startsWith("_") || ["constructor", "prototype"].includes(key)) continue;
  ...
  for (const root of roots) {
    ...
    if (value !== undefined) {
      result[root] = value;
    } else if (block.props && block.props[root] !== undefined) {
      result[root] = block.props[root];
    }
  }
  return result;
}
...
export function resolveBlock(block, ctx, opts = {}) {
  ...
  const expandedChildren = Array.isArray(block.children)
    ? expandBlocks(block.children, effectiveCtx, opts)
    : undefined;
  ...
  return {
    ...
    _resolved: true,
    _missing: missing,
    ...(expandedChildren !== undefined ? { children: expandedChildren } : {}),
...
export function expandBlocks(blocks, ctx, opts = {}) {
  ...
  for (const block of list) {
    ...
    if (repeat?.enabled && repeat.source) {
      ...
      let items = sliceItems(rawCollection, repeat);
      const alias = repeat.alias || repeat.item || inferAlias(repeat.source);

      // Evaluate item-level conditions if configured on repeat
      const repeatConds = repeat.conditions || repeat.filter;
      if (repeatConds) {
        items = items.filter((item, idx) => {
          const itemCtx = repeatContext(ctx, alias, item, idx, items.length);
          return evaluateConditions(repeatConds, itemCtx);
        });
      }
      ...
      if (repeatInnerList) {
        ...
        if (items.length === 0) {
          ...
          const emptyMode = repeat.emptyMode || (isBuilderOrPreview ? "showSample" : "preserve");
          ...
          if (opts.sampleMode === true) {
            ...
            const singleBlock = resolveBlock(
              {
                ...
                props: {
                  ...block.props,
                  [listPropKey]: [],
                  _emptyMessage: repeat.emptyText,
                },
              },
              ctx,
              opts
            ...
            // Default "preserve": keep the section shell visible with an empty list
            const singleBlock = resolveBlock(
              {
                ...block,
                props: {
                  ...block.props,
                  [listPropKey]: [],
                  ...(repeat.emptyText ? { _emptyMessage: repeat.emptyText } : {}),
                },
              },
              ctx,
              opts
        ...
        // checking template item conditions if present
        const expandedList = [];
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const childCtx = repeatContext(ctx, alias, item, i, items.length);
          if (templateItem._conditions && !evaluateConditions(templateItem._conditions, childCtx)) {
            continue;
          }
          expandedList.push(resolveProps(templateItem, childCtx, { fallbacks: block._fallbacks || {} }));
        }
      ...
      items.forEach((item, i) => {
        const childCtx = repeatContext(ctx, alias, item, i, items.length);
        if (block._conditions && !evaluateConditions(block._conditions, childCtx)) return;
        const r = resolveBlock(block, childCtx, opts);
...
export function resolveRepeater(block, ctx, opts) {
  if (block._conditions && !evaluateConditions(block._conditions, ctx)) return null;
  ...
  let items = sliceItems(readCollection(ctx, source), {
    limit: props.limit,
    offset: props.offset,
  });

  const conds = block._repeat?.conditions || block._repeat?.filter || props.conditions || props.filter;
  if (conds) {
    items = items.filter((item, idx) => {
      const itemCtx = repeatContext(ctx, itemName, item, idx, items.length);
      return evaluateConditions(conds, itemCtx);
    });
  }
