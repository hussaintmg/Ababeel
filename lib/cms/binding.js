...
export function defaultRepeat() {
  return { enabled: false, source: "", item: "item", limit: "", offset: "", emptyText: "", emptyMode: "showSample" };
}
...
export function readCollection(ctx, source) {
  ...
  // Fallback to singular/plural or case variants if raw is empty
  if ((raw === null || raw === undefined || (Array.isArray(raw) && raw.length === 0)) && ctx) {
    const s = String(source).trim().toLowerCase().replace(/[^a-z0-9]/g, "");
    const aliases = {
      courseref: ["courseReferences", "courses", "courseReference", "courseRef", "course"],
      courserefs: ["courseReferences", "courses", "courseReference", "courseRef", "course"],
      coursereference: ["courseReferences", "courses", "courseReference", "courseRef", "course"],
...
const AUTHORING_KEYS = ["_conditions", "_condProps", "_fallbacks", "_repeat", "children"];
  ...
  for (const block of list) {
    ...
    if (repeat?.enabled && repeat.source) {
      ...
      if (items.length === 0) {
        ...
        if (isBuilderOrPreview || emptyMode === "showSample") {
          const sample = createSampleItem(repeat.source, repeat.item, ctx);
          const childCtx = repeatContext(ctx, repeat.item, sample, 0, 1);
...
function resolveRepeater(block, ctx, opts) {
  ...
  if (rendered.length === 0 && (opts.isBuilder || opts.preview || props.emptyMode === "showSample")) {
