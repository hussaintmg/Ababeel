...
async function ffmpegPath() {
  if (process.env.FFMPEG_PATH && (path.isAbsolute(process.env.FFMPEG_PATH) || fs.existsSync(process.env.FFMPEG_PATH))) return process.env.FFMPEG_PATH;
  try {
    const mod = await import("ffmpeg-static");
    const raw = mod?.default ?? mod;
    const bin = typeof raw === "string" ? raw : typeof raw?.default === "string" ? raw.default : raw?.path || "";
