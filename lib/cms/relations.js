...
export async function resolveRelationsForDocs(modelName, docs, { depth = 0, seenModels = new Set(), isPublic = true } = {}) {
  ...
  for (const rel of relations) {
    ...
    for (const doc of docs) {
      if (!doc) continue;
      const val = doc[rel.sourceField] || (modelName === "CourseReference" && rel.alias === "course" ? doc.courseId : null) || (rel.isMongooseRef ? doc[rel.alias] : null);
    ...
    try {
      const targetQuery = { [rel.targetField]: { $in: validIds }, ...(isPublic ? publicRecordFilter(rel.targetModel) : {}) };
