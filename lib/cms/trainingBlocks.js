  ...
  switch (block.type) {
    ...
    case "scheduleList": {
      ...
      return {
        ...
        load: () => upcomingSessions({ mode, months, limit }),
      };
    }
...
/**
 * Sessions across the next few months, flattened and trimmed.
 *
 * A "next sessions" strip should not stop at the end of the current month —
 * on the 28th that would leave it empty — so it walks forward until it has
 * enough or runs out of months to look at.
 */
async function upcomingSessions({ mode, months, limit }) {
  const now = new Date();
  const out = [];
  ...
  for (let i = 0; i < months && out.length < limit; i += 1) {
    ...
    for (const row of rows) {
      // A session that has already started is not "upcoming"; it would
      // otherwise sit at the top of the strip for the rest of the month.
      if (row.startDate && new Date(row.startDate) < today) continue;
      if (out.some((r) => r._id === row._id)) continue;
      out.push(row);
